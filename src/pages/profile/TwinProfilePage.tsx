import { twinProfilePageData } from '../../features/profile/data/profileData';
    ...
    <div className="p-4 md:p-6">
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="rounded-2xl border border-base-300/70 bg-base-100 p-5 shadow-sm">
          <h1 className="text-3xl font-bold">{twinProfilePageData.heading}</h1>
          <p className="mt-2 text-sm text-base-content/70">{twinProfilePageData.description}</p>
        </header>
        ...
        <section className="grid gap-4 md:grid-cols-2">
          <article className="rounded-2xl border border-base-300/70 bg-base-100 p-5 shadow-sm">
            ...
            <h2 className="mt-3 text-2xl font-bold">{twinProfilePageData.academic.level}</h2>
            <p className="mt-1 text-xs text-base-content/65">
              {twinProfilePageData.academic.subtitle}
            </p>
            <div className="mt-4 space-y-2 text-sm text-base-content/75">
              {twinProfilePageData.academic.scores.map((score) => (
                <p key={score.label}>
          ...
          <article className="rounded-2xl border border-base-300/70 bg-base-100 p-5 shadow-sm">
            ...
            <h2 className="mt-3 text-2xl font-bold">
              {twinProfilePageData.preferences.profileLabel}
            </h2>
            <div className="mt-4 flex flex-wrap gap-2">
              {twinProfilePageData.preferences.tags.map((tag) => (
            ...
            <p className="mt-4 text-sm text-base-content/75">
              {twinProfilePageData.preferences.description}
            </p>
          </article>
        </section>
        ...
        <section className="grid gap-4 md:grid-cols-2">
          <article className="rounded-2xl border border-base-300/70 bg-base-100 p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-3 text-sm">
              {twinProfilePageData.recurringDifficulties.map((difficulty) => (
                <div
          ...
          <article className="rounded-2xl border border-base-300/70 bg-base-100 p-5 shadow-sm">
            ...
            <div className="mt-4 overflow-x-auto">
              <table className="table table-zebra">
                ...
                <tbody>
                  {twinProfilePageData.evaluationHistory.map((item) => (
